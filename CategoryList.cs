            ...
            while (goOn == true)
            {
                try
                {
                    ...
                    if (index == 0)
                    {
                        ...
                        {
                            Movie t = MovieList[i];
                            //This is where we are just using the override to display both the title and genre in the string listed in the override method
                            Console.WriteLine(t);
                            //This displays "Title: {Title}\nGenre: {Genre}" but can still be modified here if we wanted to look at t.title.
                            //It prevents us from needing to throw it into a string, like the cases below
                        }
                    }
                    else if (index == 1)
                    {
